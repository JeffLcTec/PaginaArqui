...
if on:
   ...
   datos['dia'] = pd.to_datetime(datos['created_at']).dt.date
    
    # Convertir la columna 'temperatura' a tipo float
   datos['field1'] = datos['field1'].astype(float)
    
    # Filtrar los datos según el rango de fechas
   datos_filtrados = datos[(datos['created_at'] >= fecha_inicio) & (datos['created_at'] <= fecha_fin)]
    
    # Calcular el promedio de temperatura en el rango de fechas
   promedio_temperatura = datos_filtrados['field1'].mean()
   promedio_humedad = datos_filtrados['field2'].mean()
    # Crear la figura del gráfico
   fig = go.Figure()
   fig1 = go.Figure() 
    # Añadir la línea de temperaturas
   fig.add_trace(go.Scatter(x=datos_filtrados['created_at'], y=datos_filtrados['field1'],mode='lines+markers', line=dict(color='red'),name='Temperatura'))
   fig.update_layout(
            ...
            xaxis=dict(tickmode='array', tickvals=datos_filtrados['created_at'])
         ) 
    # Añadir la línea del promedio
   fig.add_trace(go.Scatter(x=datos_filtrados['created_at'], y=[promedio_temperatura] * len(datos_filtrados),
   ...
   fig1.add_trace(go.Scatter(x=datos_filtrados['created_at'], y=datos_filtrados['field2'],mode='lines+markers',line=dict(color='blue'), name='Humedad'))
   fig1.update_layout(
   ...
   if fecha_inicio and fecha_fin:
       try:
           ...
           fig_temp = go.Figure()
           fig_temp.add_trace(go.Scatter(x=datos_filtrados['created_at'], y=datos_filtrados['field1'], mode='lines', name='Temperatura', line=dict(color='red')))
           ...
           fig_humedad.add_trace(go.Scatter(x=datos_filtrados['created_at'], y=datos_filtrados['field2'], mode='lines', name='Humedad', line=dict(color='blue')))
